...
def Z_y_or_Pi_valid(Z: torch.Tensor, y_or_Pi: torch.Tensor, invariance_type: str) -> bool:  # (M, dims) and (M, )
...
def Z_y_valid(Z: torch.Tensor, y: torch.Tensor, invariance_type: str) -> bool:  # (M, dims) and (M, )
    return Z_valid(Z=Z, invariance_type=invariance_type) \
           and y_valid(y) \
           and Z.shape[0] == y.shape[0]


def Z_Pi_valid(Z: torch.Tensor, Pi: torch.Tensor, invariance_type:str) -> bool:  # (M, dims) and (M, M)
    return Z_valid(Z=Z, invariance_type=invariance_type) \
           and Pi_valid(Pi) \
...
def Z_valid_order_2_no_invariance(Z: torch.Tensor, invariance_type: str) -> bool:  # (M, N)
    return len(Z.shape) == 2 \
           and InvarianceSpecification.no_invariance(invariance_type=invariance_type)


def Z_valid_order_3_shift_invariance(Z: torch.Tensor, invariance_type: str) -> bool:  # (M, C, T)
    return len(Z.shape) == 3 \
           and InvarianceSpecification.shift_invariance(invariance_type=invariance_type)


def Z_valid_order_4_translation_invariance(Z: torch.Tensor, invariance_type: str) -> bool:  # (M, C, H, W)
    return len(Z.shape) == 4 \
           and InvarianceSpecification.translation_invariance(invariance_type=invariance_type)
