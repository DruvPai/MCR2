...
class ReduNetShiftInvariantModel(pl.LightningModule):
    def __init__(self):
        super(ReduNetShiftInvariantModel, self).__init__()
        self.redu_layers = mcr2.algo.redunet.ReduBlock(*[
            mcr2.algo.redunet.layers.ShiftInvariantReduLayer(5.0, 0.5, 500.0, K, (C, T), False) for
            _ in range(L)
        ])
        self.initialized = False
        self.loss = mcr2.objective.supervised.loss.ShiftInvariantSupervisedMCR2Loss(0.5)

    def forward(self, x):
        if self.initialized:
            return F.normalize_1d(self.redu_layers(x))
        else:
...
model = ReduNetShiftInvariantModel()
