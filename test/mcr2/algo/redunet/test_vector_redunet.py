...
class ReduNetVectorModel(pl.LightningModule):
    def __init__(self):
        super(ReduNetVectorModel, self).__init__()
        self.redu_layers = mcr2.algo.redunet.ReduBlock(*[
            mcr2.algo.redunet.layers.VectorReduLayer(5.0, 0.5, 500.0, K, D, False) for _ in range(L)
        ])
    ...
    def forward(self, x):
        if self.initialized:
            return F.normalize_vec(self.redu_layers(x))
        else:
...
class SyntheticDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return {
            "full": torch.utils.data.DataLoader(self.dataset, batch_size=len(self.dataset), shuffle=True),
